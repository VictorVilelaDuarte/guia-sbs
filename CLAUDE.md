...
src/app/comerciante/
  ...
  page.tsx                — Início (entrada de todos; cartões por plano e papel — ver acima)
  ...
  gestao/page.tsx         — só redireciona para /comerciante (o antigo Resumo virou o Início)
...
- **Loaders:** `src/lib/painel/queries.ts` — um por página (vitrine: `getPerfilData`, `getFotosVitrine`, `getEventosVitrine`, `getTagsVitrine`, `getHospedagemPerfil`, `getVisitasData`; Início: `getInicioData`; gestão: `getCardapioData`, `getCatalogoData`, `getPedidosData`, `getQuartosData`) + `getPainelBase()` (ctx + dados mínimos do comércio, com `React.cache` para deduplicar layout e página). Toda página resolve o comércio por `getPainelBase()` → `getComercioCtx()`, nunca por `session.user.id`.
...
- **Onde aparece:** escolha no PDV (diálogo do item, com variação junto), no **cardápio público** (bottom sheet do produto) e no **pedido pelo QR da mesa**; exibição na conta do PDV, na cozinha (`Produção`), no cupom, na fila de pedidos, no checkout e na conta pública da mesa. Relatórios têm o bloco "Adicionais mais vendidos" (`complementosMaisVendidos`).
- **Carrinho do pedido online:** `ItemCarrinho.complementos` e o `uid` da linha incluem a assinatura dos complementos — lanche com bacon não junta com lanche sem. O checkout manda só `{ opcaoId, quantidade }`; quem soma o preço é `/api/pedidos`.
...
- **Pedido pelo QR** (`pedirNaMesa`): o cliente monta o pedido no cardápio da mesa e os itens entram na comanda **marcados como solicitação** (`PedidoItem.solicitadoPor/solicitadoEm/aprovadoEm`). **Item não aprovado não entra no total, não vai para a produção e não deixa fechar a conta** — `recalcular()`, `enviarParaProducao()` e `fecharTx()` filtram por `aprovadoEm`. O atendente confirma ou recusa no PDV (ações `aprovarPedido`/`recusarPedido`): **confirmar já manda o item para a produção** (rodada + `enviadoEm` na mesma transação — item a item vira uma rodada por vez, "confirmar e enviar" manda todos numa só); recusar apaga o item e registra no histórico. Preço e disponibilidade sempre do banco (só cardápio digital disponível); limites de 20 linhas, 10 por item e teto por janela de 5 min; `pg_advisory_xact_lock` por mesa evita que dois celulares abram duas contas.
- **Avisos do salão:** `GET /api/comerciante/gestao/chamados` devolve `{ chamados, solicitacoes }` — o PDV faz um polling só, bipa quando chega algo novo e mostra os dois na aba Comandas.
- **Mapa do salão:** a aba **Comandas** do PDV é o mapa (`mapa-salao.tsx`), alimentado por `mapaDoSalao()` — mesa ativa com conta aberta (valor, tempo, itens na cozinha, pedidos do QR esperando), chamado pendente e o estado **"a liberar"** (`Mesa.liberarAte`, 15 min, gravado ao fechar a conta em `fecharTx`). Tocar numa mesa livre abre a conta direto com a taxa de serviço padrão; contas sem mesa cadastrada (`Pedido.mesaId` nulo) ficam no grupo "Outras contas". `GET /api/comerciante/gestao/comandas` devolve `{ comandas, mesas }`.
- **Planta do salão (§16.1 do plano):** `Mesa.posX`/`posY` guardam a posição em **células** de uma grade `14 × 10` (`src/lib/gestao/mesas-grade.ts`, sem runtime) — **nunca pixels**, que quebram quando a tela muda. Posição é **por área** (Salão e Varanda são plantas independentes; colisão só dentro da mesma área) e soltar em cima de outra mesa encosta na célula livre mais próxima (`celulaLivre()`, busca em espiral). `posX/posY` nulos = fora da planta (bandeja "Sem posição" no editor, cartões abaixo da planta no PDV). Editor arrastável em `mesas/mapa-editor.tsx` (pointer events, mouse e dedo, salvamento otimista) → `POST /api/comerciante/gestao/mesas/posicoes` (`pedidos:configurar`, `{ posicoes }` ou `{ tirar }`) → `posicionarMesas()`/`tirarDaPlanta()`. No PDV a planta só desenha o **retângulo usado**, com célula responsiva (mín. 76 px, cartão compacto abaixo de 108 px, rolagem horizontal só quando não couber) e botão **Planta / Lista** com a escolha no `localStorage` (`pdv:mapa-modo`); loja sem planta montada continua vendo a lista.
- **Permissão de liberar:** `PATCH /api/comerciante/gestao/mesas/[id]` exige `pedidos:configurar`, **exceto** quando o corpo é só `{ liberar: true }` — aí basta `vendas:registrar`, porque quem limpa a mesa é o salão, não quem configura.
- **Cadastro:** `lugares` (opcional, aparece no mapa); a ordem da lista deixou de ser arrastável — quem organiza o salão é a planta (`Mesa.ordem` segue só como desempate).
...
**`Featured`** (`src/components/public/home/featured.tsx`) recebe `items: ComercioDestaque[]`. Mostra comércios com `destaque_busca: true` no plano. Foto de capa: `fotos[0]` → `logo` → `PhotoPH`. Retorna `null` quando vazio. Card é `<Link>` para a vitrine.
...
Feature controlada pelo plano (`key: "catalogo"`). Produtos e serviços são instâncias do mesmo model `Produto`, diferenciados pelo campo `tipo: TipoProduto` (PRODUTO | SERVICO). **Cardápio e catálogo são independentes (caminho A, 2026-09-24):** no cardápio = `categoriaCardapioId` preenchido; no catálogo = `noCatalogo = true`. O mesmo produto pode estar nos dois (Água mineral da demo), em um só ou em nenhum (**só PDV** — sacola, gelo). A tela **Produtos e serviços** lista o que está no catálogo ou só no PDV (item só do cardápio fica na tela Cardápio); a tela Cardápio marca "Também no catálogo". O limite do plano Gratuito conta os itens da tela Produtos e serviços (`negarLimiteCatalogo`). **Pedido online aceita produto do catálogo** (tipo PRODUTO; serviço não vai pelo carrinho) — a página `/catalogo` tem carrinho e `CartBar` como o cardápio, e o checkout volta para o cardápio ou para o catálogo (`voltarHref`) conforme a loja tenha itens no cardápio. O `BottomNav` some em `.../catalogo` (colidiria com a barra do carrinho). Migração: `prisma/migrate-onde-aparece.ts`.
...
- **Onde aparece:** ver "Catálogo de produtos e serviços" — cardápio (`categoriaCardapioId`) e catálogo (`noCatalogo`) independentes; nenhum dos dois = só PDV. (A chave `mostrarNaVitrine`, de 2026-09-24, foi substituída por `noCatalogo` no mesmo dia.) `arquivado = true` → fora de linha: some de tudo (inclusive PDV); nas listas do painel vai para o bloco recolhido "Arquivados". **Consulta pública nova de produto filtra `arquivado: false`** e o destino (`noCatalogo: true` no catálogo; categoria do cardápio no cardápio); venda do PDV recusa arquivado em `resolverItens()`.
...
- **Deploy:** `npm run db:push` (aditivo: colunas novas, enum `UnidadeProduto`, índices únicos por loja) **antes** de publicar — as consultas novas usam as colunas. Para o `noCatalogo`: `npx tsx prisma/migrate-onde-aparece.ts` (cria e preenche a coluna) → publicar → `npm run db:push` (remove `mostrarNaVitrine`).
...
- **Módulo de Gestão** — separa o painel em "Minha vitrine" (Guia) e "Gestão" (operação) e
  evolui a gestão para equipe com papéis, clientes (CRM leve), venda manual + relatórios e
...
- ~~Produto simultâneo no cardápio e no catálogo~~ — implementado (`noCatalogo`, ver Catálogo de produtos e serviços)
