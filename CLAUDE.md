...
- **Mapa do salão:** a aba **Comandas** do PDV é o mapa (`mapa-salao.tsx`), alimentado por `mapaDoSalao()` — uma carta por mesa ativa, agrupada por área, com conta aberta (valor, tempo, itens na cozinha, pedidos do QR esperando), chamado pendente e o estado **"a liberar"** (`Mesa.liberarAte`, 15 min, gravado ao fechar a conta em `fecharTx`). Tocar numa mesa livre abre a conta direto com a taxa de serviço padrão; contas sem mesa cadastrada (`Pedido.mesaId` nulo) ficam no grupo "Outras contas". `GET /api/comerciante/gestao/comandas` devolve `{ comandas, mesas }`.
...
- **Cadastro:** `lugares` (opcional, aparece no mapa) e ordem por arrastar (`POST .../mesas/ordem`, `pedidos:configurar`).
