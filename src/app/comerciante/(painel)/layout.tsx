...
// Shell do painel do comércio: menu lateral agrupado no desktop, barra inferior
// (4 atalhos + "Mais") no celular — ver painel-nav.tsx.
...
export default async function ComercianteLayout({
  ...
  // Conta e saída — no rodapé da sidebar e do "Mais". Admin não ganha "Sair":
  // deslogaria a sessão de admin.
  const rodape = (
    <div className="space-y-1.5">
      ...
      {isAdmin && comercio ? (
        <Link
          ...
          className="-ml-2 inline-flex items-center gap-1 rounded-md px-2 py-1.5 text-sm font-medium hover:bg-accent transition-colors"
        >
        ...
        <div className="-ml-2 flex flex-wrap items-center gap-1">
          <Link
            ...
            className="inline-flex items-center gap-1.5 rounded-md px-2 py-1.5 text-sm text-muted-foreground hover:bg-accent hover:text-foreground transition-colors"
          >
  ...
  // Nome da loja (seletor quando há várias), situação e plano.
  const topo = (
    <div className="min-w-0 space-y-1.5">
      <SeletorLoja lojas={base?.lojas ?? []} atualId={comercio.id} nomeAtual={comercio.nome} compacto />
      <div className="flex flex-wrap items-center gap-1.5">
        <Badge variant={statusVariants[comercio.status]}>{statusLabels[comercio.status] ?? comercio.status}</Badge>
        <Badge variant={comercio.plan.slug === "premium" ? "default" : "outline"}>{comercio.plan.nome}</Badge>
      </div>
    </div>
  )
  ...
  return (
    <PedidosAlertaProvider
      ...
      key={comercio.id}
      ativo={temFeature(comercio.plan.features, "pedido_online") && temPermissao(permissoes, "pedidos:operar")}
    >
      <div className="min-h-screen bg-muted/30">
        <PainelSidebar nav={nav} topo={topo} rodape={rodape} />

        {/* Celular: só o nome da loja no topo; o menu está na barra de baixo. */}
        <header className="sticky top-0 z-30 border-b bg-background/95 px-4 py-3 backdrop-blur md:hidden print:hidden">
          <SeletorLoja lojas={base?.lojas ?? []} atualId={comercio.id} nomeAtual={comercio.nome} compacto />
        </header>
        ...
        <div className="md:pl-64 print:pl-0">
          <main className="mx-auto max-w-5xl space-y-6 px-4 py-6 md:px-8 md:py-8">
            {isAdmin && (
              <div className="flex items-start gap-3 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-amber-900 print:hidden">
        ...
        <PainelBottomNav nav={nav} topo={topo} rodape={rodape} />
