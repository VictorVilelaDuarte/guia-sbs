...
export function TagsEditor({ tagsIniciais, limite }: TagsEditorProps) {
  ...
  async function addTag(raw: string) {
    ...
    if (atingiuLimite) {
      toast.warning(
        `Limite de ${limite} palavras-chave atingido. Faça upgrade para o plano Premium.`,
      );
      setInput("");
      return;
    }
  ...
  async function removeTag(id: string) {
    setRemovingId(id);
    const res = await fetch(`/api/comerciante/tags/${id}`, {
      method: "DELETE",
    });
    setRemovingId(null);
    ...
    <div className="space-y-3">
      ...
      >
        ...
        {!atingiuLimite && (
          <input
            ...
            onBlur={() => {
              if (input.trim() && !addingRef.current) addTag(input);
            }}
            placeholder={
              tags.length === 0
      ...
      {atingiuLimite && (
        <p className="text-xs text-amber-600 font-medium">
          Limite de {limite} palavras-chave atingido. Faça upgrade para o plano
          Premium para adicionar mais.
        </p>
      )}
        ...
        <p className="text-xs text-muted-foreground">
          {!atingiuLimite && (
            <>
              ...
              para adicionar. Backspace remove a última.
            </>
          )}
        </p>
        {limite !== undefined && (
          <p
            className={`text-xs font-medium ${atingiuLimite ? "text-amber-600" : "text-muted-foreground"}`}
          >
